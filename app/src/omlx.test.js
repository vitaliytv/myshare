...
describe('translateChunk', () => {
  test('POST /v1/chat/completions, повертає trimmed контент', async () => {
    ...
    const out = await translateChunk('Hello world', { model: 'm', base: 'http://x/v1' })
  ...
  test('apiKey → Authorization: Bearer', async () => {
    ...
    await translateChunk('hi', { model: 'm', base: 'http://x/v1', apiKey: 'secret' })
  ...
  test('без apiKey — без Authorization', async () => {
    fetchMock.mockResolvedValueOnce(chatOk('ok'))
    await translateChunk('hi', { model: 'm', base: 'http://x/v1' })
    const [, init] = fetchMock.mock.calls[0]
  ...
  test('не-ok → кидає помилку з кодом', async () => {
    ...
    await expect(translateChunk('hi', { base: 'http://x/v1' })).rejects.toThrow('omlx HTTP 500')
  })
})
...
describe('translateToUkrainian', () => {
  test('запити несуть temperature 0.2', async () => {
    ...
    await translateToUkrainian('hello', { model: 'm', base: 'http://x/v1' })
    const chatCall = fetchMock.mock.calls.find((c) => c[0] === 'http://x/v1/chat/completions')
  ...
  test('без model — спершу resolveModel через /v1/models', async () => {
    ...
    const result = await translateToUkrainian('hello', { base: 'http://x/v1' })
    expect(result.model).toBe('auto-model')
    expect(fetchMock.mock.calls[0][0]).toBe('http://x/v1/models')
    expect(fetchMock.mock.calls[1][0]).toBe('http://x/v1/chat/completions')
  })
})
