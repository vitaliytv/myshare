...
// fetch-відповідь omlx /v1/chat/completions
function chatOk(message) {
  return { ok: true, json: () => Promise.resolve({ choices: [{ message }] }) }
}
...
describe('runAgent', () => {
  test('викликає tool, згодовує конверт, завершує на текстовій відповіді', async () => {
    ...
    const dispatch = vi.fn().mockResolvedValue({ ok: true, output: { videoId: 'abcdefghijk' } })
  ...
  test('зупиняється на maxSteps якщо модель не перестає кликати tool', async () => {
    const chat = vi.fn().mockResolvedValue(assistantCall('youtube_id', { url: 'https://youtu.be/abcdefghijk' }))
...
describe('createOpenAiChat', () => {
  test('POST-ить messages+tools і повертає assistant message', async () => {
    const fetchFn = vi.fn().mockResolvedValue(chatOk(assistantText('hi')))
...
describe('createLiteRtChat', () => {
  test('кличе Tauri-команду litert_chat і повертає message', async () => {
    const invoke = vi.fn().mockResolvedValue(assistantText('привіт'))
...
describe('selectChat', () => {
  test('android → LiteRT-LM (invoke)', async () => {
    const invoke = vi.fn().mockResolvedValue(assistantText('a'))
    const fetchFn = vi.fn()
  ...
  test('desktop → omlx (fetch)', async () => {
    const invoke = vi.fn()
    const fetchFn = vi.fn().mockResolvedValue(chatOk(assistantText('b')))
...
describe('createSystemPrompt', () => {
  test('згадує домен і tool-дисципліну', () => {
    ...
    expect(p).toMatch('one tool at a time')
  })
})
