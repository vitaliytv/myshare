...
export const TOOLS = [
  {
    ...
    name: 'youtube_id',
    summary:
      'Extract the 11-char YouTube video id from a URL. Returns empty when the URL is not a recognizable YouTube link.',
    input: {
  ...
  },
  {
    ...
    input: {
      videoId: {
        type: 'string',
        required: true,
        description: '11-char YouTube video id (use the youtube_id tool to get it).'
      }
    },
  ...
  },
  {
    ...
    input: {
      ...
      preferred: {
        type: 'array',
        required: false,
        description: 'Language codes in priority order, e.g. ["uk","en"]. Defaults to ["uk","en"].'
      }
    },
  ...
  },
  {
    ...
    name: 'translate',
    summary:
      'Translate English text into Ukrainian via the local LLM. Returns { model, text, segments } with aligned original↔translation pairs.',
    input: {
      text: { type: 'string', required: true, description: 'Source text in English.' },
      model: {
        type: 'string',
        required: false,
        description: 'Preferred model id; falls back to the first loaded model.'
      }
    },
    // ctx несе UI-афорданси поза JSON-схемою (onProgress, signal) — LLM-шлях їх
    // не передає, маніфест їх не бачить.
    run: (input, ctx = {}) =>
      translateToUkrainian(input.text, { model: input.model, onProgress: ctx.onProgress, signal: ctx.signal })
  },
  {
    tier: 'read',
    name: 'list_links',
