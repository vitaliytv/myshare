...
- **SUPADATA_API_KEY** — Rust-константа в `app/src-tauri/src/youtube.rs` із supadata API ключем, який команда `yt_get_transcript` додає у `x-api-key` header. Захардкоджена для нульового setup'у; ротація = правка константи + cargo rebuild.
...
- **omlx** — локальний OpenAI-compatible MLX-сервер на Apple Silicon (`http://127.0.0.1:8000/v1`), який Frontend `myshare` (desktop-only) використовує для перекладу EN→UA YouTube-субтитрів через `POST /v1/chat/completions` і `GET /v1/models`. Замість Ollama: той самий протокол, що й у task-app, портативна логіка перекладу. Модуль `app/src/omlx.js`, ключ моделі в localStorage — `myshare.omlxModel`.
