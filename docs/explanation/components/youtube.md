...
## Переклад EN→UA через локальний Ollama (desktop)

Коли українських субтитрів нема, а англійські є (статус **🇬🇧 EN**), на **desktop** (Mac) поряд зʼявляється кнопка **«Перекласти»**. Вона тягне англійський транскрипт і перекладає його українською локальною LLM через **Ollama** — без хмари, без ключів, безкоштовно.

- **Чому Ollama, а не хмарний переклад**: переклад великих транскриптів хмарними API коштує грошей і шле приватний контент назовні. Ollama крутиться локально на Mac розробника; на Android кнопки нема (`canTranslate = !isAndroid`).
- **HTTP напряму з JS**: Ollama слухає `http://localhost:11434`, тож звертаємось через `tauri-plugin-http` (як `page-meta.js`) — окрема Rust-команда не потрібна.
- **Чанкінг**: транскрипт ріжемо на фрагменти (`chunkText`, ≤3500 символів по межах абзаців/рядків) і перекладаємо послідовно `POST /api/chat` (`stream:false`, `temperature:0.2`, `num_ctx:8192`, `keep_alive:'5m'`). Прогрес `done/total` показується у діалозі.
- **Модель**: дефолт `gemma3:4b` (~3.3 GB — влазить у RAM без свопу навіть із dev-оточенням; 8B-моделі типу `gemma4:e4b` на 16 GB ганяють своп і гальмують систему). `resolveModel` через `GET /api/tags` бере дефолт, якщо завантажений, інакше першу наявну (працює «з коробки» з будь-якою моделлю).
- **Памʼять**: `num_ctx:8192` тримає KV-кеш малим; `keep_alive:'5m'` лишає модель завантаженою 5 хв, щоб наступні чанки/повторні переклади не вантажили її щоразу.
- **Кеш**: результат (`{model, originalLang, segments}`, де `segments` — пари `original↔translated`) кешується у `localStorage` за `videoId` (ключ `myshare.translations`). Повторний тап відкриває переклад миттєво — Ollama не запускається вдруге.
...
- **Помилки**: якщо Ollama не запущено, fetch падає → у діалозі підказка `ollama serve`.

??? engineer "Реалізація перекладу"
    - JS: `app/src/ollama.js` — `chunkText`, `buildMessages`, `extractContent` (чисті), `translateChunk` (`keep_alive:'5m'`, `num_ctx:8192`), `listOllamaModels`, `resolveModel`, `translateToUkrainian({onProgress})`. HTTP через `@tauri-apps/plugin-http`.
    ...
    - Тести: `ollama.test.js` (chunker, prompt, parsing, resolveModel, orchestration з mock fetch), `translation-cache.test.js` (валідація/round-trip).
