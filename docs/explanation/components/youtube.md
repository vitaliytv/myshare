...
- **Чанкінг**: транскрипт ріжемо на фрагменти (`chunkText`, ≤3500 символів по межах абзаців/рядків) і перекладаємо послідовно `POST /api/chat` (`stream:false`, `temperature:0.2`). Прогрес `done/total` показується у діалозі.
- **Модель**: дефолт `gemma4:e4b`; `resolveModel` через `GET /api/tags` бере дефолт, якщо завантажений, інакше першу наявну (працює «з коробки» з будь-якою моделлю).
...
??? engineer "Реалізація перекладу"
    - JS: `app/src/ollama.js` — `chunkText`, `buildMessages`, `extractContent` (чисті), `translateChunk`, `listOllamaModels`, `resolveModel`, `translateToUkrainian({onProgress})`. HTTP через `@tauri-apps/plugin-http`.
